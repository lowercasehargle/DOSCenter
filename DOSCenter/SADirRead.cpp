...
bool CSADirRead::GetDirs(CString pDirPath, bool bRecurse)
{
...
bool CSADirRead::GetFiles(CString pFilemask, bool bIncludeFilesInFileList, bool bIncludeFoldersInFileList)
{
   ...
   HANDLE hFind;
	try 
	{
		if ((hFind = FindFirstFile(fullPath, &FindFileData))!=INVALID_HANDLE_VALUE)
		{
         ...
         // skip . and ..
         if (bIsFolder && (wcscmp(FindFileData.cFileName, L".")==0) || (wcscmp(FindFileData.cFileName, L"..")==0))
         {
            bAddThisOne = false;
         }
