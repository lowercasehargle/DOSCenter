...
//
HZIP XOpenZip(void *z, unsigned int len, DWORD flags);
// XOpenZip - opens a zip file and returns a handle with which you can
// subsequently examine its contents. You can open a zip file from:
// from a pipe:             XOpenZip(hpipe_read,0, ZIP_HANDLE);
// from a file (by handle): XOpenZip(hfile,0,      ZIP_HANDLE);
// from a file (by name):   XOpenZip("c:\\test.zip",0, ZIP_FILENAME);
// from a memory block:     XOpenZip(bufstart, buflen, ZIP_MEMORY);
...
// Returns:     ZRESULT - ZR_OK if success, otherwise some other value
//
ZRESULT XCloseZip(HZIP hz);
// XCloseZip - the zip handle must be closed with this function.
...
#define ZR_BADCOMP	  0x00000900     // compression method unsupported
...
// HZIP hz = XOpenZip("c:\\stuff.zip",0,ZIP_FILENAME);
...
// HZIP hz = XOpenZip(zipbuf, ziplen, ZIP_MEMORY);
...
// HZIP hz = XOpenZip(hread,0,ZIP_HANDLE);
// for (int i=0; ; i++)
...
HZIP XOpenZipU(void *z,unsigned int len,DWORD flags);
ZRESULT XCloseZipU(HZIP hz);
unsigned int FormatZipMessageU(ZRESULT code, char *buf,unsigned int len);
bool IsZipHandleU(HZIP hz);
#define XOpenZip XOpenZipU

#ifdef XZIP_H
#undef XCloseZip
#define XCloseZip(hz) (IsZipHandleU(hz)?XCloseZipU(hz):XCloseZipZ(hz))
#else
#define XCloseZip XCloseZipU
