...
public:
	...
	int __inline	SetOwner(CRegistry* Owner) { __cregOwner = Owner; }
	...
	int __inline	IsString()		{ return (iType == REG_SZ); }
	int __inline	IsDWORD()		{ return (iType == REG_DWORD); }
	int __inline	IsBinary()		{ return (iType == REG_BINARY); }	
	int __inline	IsMultiString() { return (iType == REG_MULTI_SZ); }
	
	int __inline	IsStored()		{ return __bStored; }
	int __inline	Exists()		{ return __bStored; }

	int __inline	MultiClear()	{ SetMulti(_T("\0"), 2); }
	int __inline	MultiAdd(LPCTSTR lpszVal) { MultiSetAt(MultiCount(), lpszVal); }
