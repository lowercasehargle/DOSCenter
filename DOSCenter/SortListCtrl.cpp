...
void CSortListCtrl::Sort( int iColumn, BOOL bAscending )
{
    ...
    for (int i=0;i < this->GetItemCount();i++)//explicitly set lparam
    {
...
void CSortListCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
 ...
 switch(lplvcd->nmcd.dwDrawStage)
 {
   ...
   *pResult = CDRF_DODEFAULT;
   	...
   	tmp=CListCtrl::GetItemText( lplvcd->nmcd.dwItemSpec, 0); //( nItem, nSubItem, lpszText ) )
	// hijacking color column to also hide a .dat index.  If it's there, strip it out when we're displaying this row's color.
	int pos = tmp.Find(L"|");
	if (pos != -1)
		tmp = tmp.Left(pos);

	swscanf_s(tmp,L"%x",&color);
 //  int iRow = lplvcd->nmcd.dwItemSpec;
  // if(iRow & 1)
  // {
   // lplvcd->clrTextBk = RGB(255, 0, 0);
    lplvcd->clrText = color; //RGB( 0, (BYTE)color, 0);
