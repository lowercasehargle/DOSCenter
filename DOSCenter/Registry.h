...
#define REGENTRY_CONV_NUMERIC_STORAGETYPE(type, maxlen, form, from_sz, from_dw) \
	REGENTRY_CONV_STORAGETYPE(type, _R_BUF(maxlen); swprintf_s(buffer, _T(#form), Value);, from_sz, from_dw, 0)
...
public:
	...
	CRegEntry*	GetAt(size_t n) { assert((int)n < Count());  return _reEntries.at(n); }
	...
	int __inline	GetFlags()	{	return __dwFlags; }
	int __inline	Count()		{	return _reEntries.size(); }
